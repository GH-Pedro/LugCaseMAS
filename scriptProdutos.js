...
function ViewModel(){
    ...
    self.isValidDay = function() {
        if(/^(0[1-9]|[12][0-9]|3[01])\/(0[1-9]|1[012])\/(202[3-9]|20[3-9][0-9]|21[0-9][0-9])$/.test(self.diaRecolha())){
            return true
        }
        else{
            return false
    ...
    self.checkBagagens = ko.computed(function(){
        ...
        for(i = 0; i < self.bagagens().length; i++){
            ...
            else if(!self.isValidPeso(self.bagagens()[i].Peso()) || !self.isValidDim(self.bagagens()[i].Dim())){
                return false
            }
        }
        return true
    ...
    self.confirmPlaces = function() {
        ...
        if(self.number() == 2){
            self.precoBagagens(1.20);
        }
        if(self.number() == 3){
            self.precoBagagens(1.30);
        }
        var pesoTotal = 0;
    ...
    self.coordsRecolha.subscribe(function(newPosition) {
        markerRecolha.clearLayers();  
        L.marker(newPosition, {title: self.confirmRecolha(), icon: greenMarkerIcon}).addTo(markerRecolha).bindPopup(self.confirmRecolha()+" - "+" Local de Recolha").openPopup();
    });
    ...
    self.coordsEntrega.subscribe(function(newPosition) {
        markerEntrega.clearLayers();
      L.marker(newPosition, {title: self.confirmEntrega(), icon: redMarkerIcon}).addTo(markerEntrega).bindPopup(self.confirmEntrega()+" - "+" Local de Entrega").openPopup();
    });
    ...
    self.precofinal = ko.computed(function(){
        return (self.precoBagagens()*self.precoPeso()*self.precoDistancia()*self.precoPrioridade()).toFixed(2);
    })
    ...
    self.coordsEntrega.subscribe(function(){
      ...
      self.checkForm1 = ko.computed(function(){
        ...
        if( self.localEntrega == "" || self.localRecolha == "" || self.diaRecolha == "" || self.horaRecolha == ""){
            return false
        }
        else if( !self.checkRecolha() || !self.checkEntrega() || !self.isValidDay() || !self.isValidTime()){
            return false
        }
